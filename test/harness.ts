...
export function check(a: Assertion | LimbAssertion | BrainAssertion | VisionAssertion | LanguageAssertion, f: Findings): string[] {
  ...
  switch (a.kind) {
    ...
    case 'hemiballismus':
      for (const x of sidesOf(a.side)) {
        const got = f.hemiballismus[x];
        if (miss(got, a.oneOf)) out.push(`hemiballismus ${x}: got ${got}, expected ${show(a.oneOf)}`);
      }
      break;
    case 'truncal_ataxia':
      ...
      break;
    case 'gerstmann':
      if (miss(f.gerstmann, a.oneOf)) out.push(`gerstmann: got ${f.gerstmann}, expected ${show(a.oneOf)}`);
      break;
    case 'language': {
...
export const TERRITORY_CASE: Readonly<Record<string, string>> = {
  ...
  // P14.
  trochlear_nucleus: 'trochlear-nucleus-left',
  midpontine_tegmentum: 'midpontine-tegmentum-left',
  // P15.
  subthalamic_nucleus: 'subthalamic-nucleus-left',
  // P16.
...
export function territoryFailures(kb: Kb): Failure[] {
  ...
  for (const [territory, row] of Object.entries(kb.brain.territories)) {
    ...
    const kase = BRAIN_CASES.find((c) => c.id === id) ?? LANGUAGE_CASES.find((c) => c.id === id) ?? CEREBELLUM_CASES.find((c) => c.id === id) ?? POSTERIOR_CASES.find((c) => c.id === id) ?? MIDBRAIN_CASES.find((c) => c.id === id) ?? NERVE_CASES.find((c) => c.id === id) ?? BASAL_CASES.find((c) => c.id === id) ?? CORTEX_CASES.find((c) => c.id === id);
    ...
    // P12: every further brain level the case lesions must be one the territory names, and the reverse.
